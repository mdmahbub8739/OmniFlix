...
(() => {
  ...
  const STORE_FAVS     = 'omniflixFavorites';
  const STORE_ACCENT   = 'omniflixAccent';
  const STORE_PIP      = 'omniflixPipEnabled';      // 'on' | 'off'
  ...
  // NSFW / Adult content allowed app-wide. include_adult=true unlocks 18+ titles
  // in discover & search queries. Anime is treated as a first-class category.
  const ALLOW_ADULT = true;
  const ADULT_PARAM = ALLOW_ADULT ? 'include_adult=true' : 'include_adult=false';

  const SHELVES = [
    { key: 'cerebral', title: 'Mind-bending <em>thrillers</em>', sub: 'Films that refuse to leave your head.',
      query: () => `${TMDB_BASE}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=53,9648&sort_by=vote_count.desc&vote_count.gte=2000&${ADULT_PARAM}` },
    { key: 'romance',  title: 'Slow-burn <em>romance</em>',      sub: 'Long looks, longer silences.',
      query: () => `${TMDB_BASE}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=10749&sort_by=vote_average.desc&vote_count.gte=400&${ADULT_PARAM}` },
    { key: 'saturday', title: 'Loud, dumb, <em>fun</em>',        sub: 'Pure Saturday-night escapism.',
      query: () => `${TMDB_BASE}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=28&sort_by=popularity.desc&${ADULT_PARAM}` },
    { key: 'horror',   title: 'Lights off, <em>volume up</em>',  sub: 'Modern horror that hits.',
      query: () => `${TMDB_BASE}/discover/movie?api_key=${TMDB_API_KEY}&with_genres=27&sort_by=vote_count.desc&vote_count.gte=800&${ADULT_PARAM}` },
    { key: 'anime',    title: 'From <em>Japan</em>, with feeling', sub: 'Anime films & series, hand-picked.',
      query: () => `${TMDB_BASE}/discover/tv?api_key=${TMDB_API_KEY}&with_original_language=ja&with_genres=16&sort_by=popularity.desc&${ADULT_PARAM}` },
    { key: 'classics', title: 'Untouchable <em>classics</em>',  sub: 'Films you should know by heart.',
      query: () => `${TMDB_BASE}/discover/movie?api_key=${TMDB_API_KEY}&sort_by=vote_average.desc&vote_count.gte=5000&primary_release_date.lte=2005-01-01&${ADULT_PARAM}` },
    { key: 'now',      title: 'In <em>theaters</em> now',         sub: 'Currently playing on big screens.',
      query: () => `${TMDB_BASE}/movie/now_playing?api_key=${TMDB_API_KEY}&${ADULT_PARAM}` },
    { key: 'after-dark', title: 'After <em>dark</em>',           sub: 'Adults-only · 18+ cinema.',
  ...
  function personCard(p, opts = {}) {
    ...
    const tag = opts.tag ? `<span class="person-card__tag">${html(opts.tag)}</span>` : '';
  ...
  async function renderHome() {
    ...
    const renderVibeCard = (s, i, bg) => `
      <a class="vibe-card" href="/genre/movie/0/${encodeURIComponent(s.key)}"
  ...
  // ───── ANIME ──────────────────────────────────────────────────────────────────
  // Dedicated anime library. Tabs: All · TV series · Films · Genres.
  // NSFW/ecchi shelf is included (allow_adult=true app-wide).
  let animeState = { tab: 'all', sort: 'popularity.desc', page: 1, hasMore: true };

  async function renderAnime() {
    animeState = { tab: 'all', sort: 'popularity.desc', page: 1, hasMore: true };
    $('#view').innerHTML = `
      <header class="page-header page-header--anime">
        <span class="eyebrow page-header__eyebrow"><span class="dot"></span> アニメ · Anime library</span>
        <h1 class="page-header__title">Welcome to <em>Anime</em></h1>
        <p class="page-header__sub">A first-class library — TV, films, ecchi & every shade between. NSFW unlocked.</p>
      </header>

      ${ANIME.rails.map(r => `
        <section class="section ${r.key==='ecchi' ? 'section--ecchi' : ''}">
          <header class="section__head"><div>
            <h2 class="section__title">${r.title}</h2>
            <div class="section__sub">${r.sub}</div>
          </div></header>
          <div class="rail" id="animeRail-${r.key}"><div class="rail__strip">${Array.from({length:7}, skTitleCard).join('')}</div></div>
        </section>`).join('')}

      <header class="page-header" style="margin-top:32px">
        <h2 class="page-header__title" style="font-size:clamp(28px,4vw,40px)">Browse <em>all anime</em></h2>
        <p class="page-header__sub">Filter by type & sort to explore the full catalogue.</p>
      </header>
      <div class="filter-bar">
        <span class="filter-bar__label">Type</span>
        <button class="chip active" data-tab="all">All</button>
        <button class="chip" data-tab="tv">TV series</button>
        <button class="chip" data-tab="movie">Films</button>
        <span class="filter-bar__label" style="margin-left:auto">Sort</span>
        <select id="animeSort" class="chip" style="background:var(--bg-2);color:var(--text);padding-right:36px;">
      ...
      <section class="section">
        <div class="browse-grid" id="animeGrid"></div>
        <div class="load-more"><button class="btn-ghost" id="loadMoreAnime"><i class="ri-add-line"></i> Load more</button></div>
      </section>
      ${footer()}
    `;

    // Hydrate rails in parallel. Combine TV + films per rail for variety.
    Promise.all(ANIME.rails.map(r =>
      Promise.all([
        fetch(ANIME.tvQuery(r.params)).then(x => x.json()).catch(() => ({ results: [] })),
        fetch(ANIME.moviesQuery({ ...r.params, sort_by: (r.params.sort_by||'').replace('first_air_date','release_date') })).then(x => x.json()).catch(() => ({ results: [] }))
      ])
    )).then(results => {
      ANIME.rails.forEach((r, i) => {
        const [tvData, mvData] = results[i];
        const merged = []
          .concat((tvData.results || []).map(x => ({ ...x, media_type: 'tv' })))
          .concat((mvData.results || []).map(x => ({ ...x, media_type: 'movie' })))
        ...
        const rail = $(`#animeRail-${r.key} .rail__strip`);
        if (rail) rail.outerHTML = `<div class="rail__strip">${out.slice(0, 18).map(titleCard).join('')}</div>`;
      });
    });

    $$('.filter-bar [data-tab]').forEach(b => b.addEventListener('click', () => {
      $$('.filter-bar [data-tab]').forEach(x => x.classList.remove('active'));
      b.classList.add('active');
      animeState.tab = b.dataset.tab;
      animeState.page = 1;
      $('#animeGrid').innerHTML = '';
      loadAnimeGrid();
    }));
    $('#animeSort').addEventListener('change', e => {
      animeState.sort = e.target.value;
      animeState.page = 1;
      $('#animeGrid').innerHTML = '';
      loadAnimeGrid();
    });
    $('#loadMoreAnime').addEventListener('click', loadAnimeGrid);
    loadAnimeGrid();
  }
  ...
  async function loadAnimeGrid() {
    ...
    grid.insertAdjacentHTML('beforeend',
      Array.from({length:12}, () => `<div class="sk-card sk-card--inline"><div class="sk sk--poster"></div><div class="sk sk--line sk--line-lg"></div><div class="sk sk--line sk--line-sm"></div></div>`).join('')
    );

    const { tab, sort, page } = animeState;
    const queries = [];
    if (tab === 'all' || tab === 'tv') {
      const sortKey = sort === 'release_date.desc' ? 'first_air_date.desc' : sort;
      queries.push(fetch(ANIME.tvQuery({ sort_by: sortKey, page, 'vote_count.gte': sort==='vote_average.desc' ? 100 : 0 })).then(r => r.json()).then(d => ({ data: d, type: 'tv' })));
    }
    if (tab === 'all' || tab === 'movie') {
      const sortKey = sort === 'first_air_date.desc' ? 'release_date.desc' : sort;
      queries.push(fetch(ANIME.moviesQuery({ sort_by: sortKey, page, 'vote_count.gte': sort==='vote_average.desc' ? 100 : 0 })).then(r => r.json()).then(d => ({ data: d, type: 'movie' })));
    }
    ...
    responses.forEach(({ data, type }) => {
      ...
      (data?.results || []).forEach(r => { if (r.poster_path) merged.push({ ...r, media_type: type }); });
    });
    grid.insertAdjacentHTML('beforeend', merged.map(titleCard).join(''));
    animeState.hasMore = page < maxPages;
    animeState.page += 1;
    $('#loadMoreAnime').style.display = animeState.hasMore ? '' : 'none';
  }
  ...
  async function renderPersonDetail(id) {
    ...
    $('#view').innerHTML = `
      ...
      <section class="section">
        ...
        <div class="browse-grid">${cast.slice(0, 30).map(c => titleCard({ ...c })).join('')}</div>
      </section>
      ...
      ${crew.length ? `<section class="section">
        ...
        <div class="browse-grid">${crew.slice(0, 30).map(c => titleCard({ ...c })).join('')}</div>
      </section>` : ''}
  ...
  const mini = {
    ...
    // Also clamps height against the available viewport (minus bottom-nav).
    clampSize(mw) {
      ...
      const maxH = vh - (isMobile ? (this._bottomNavH() + 16) : 24);
      if (mh > maxH) { mh = maxH; mw = Math.round(mh * 16 / 9); }
      return { w: Math.round(mw), h: Math.round(mh) };
    },

    // Clamp a mini-player position so it stays inside the viewport and above
    // the bottom-nav.
    clampPos(x, y, w, h) {
      const vw = window.innerWidth, vh = window.innerHeight;
      const isMobile = vw < 860 || (matchMedia && matchMedia('(pointer: coarse)').matches);
      ...
      return {
        ...
        y: Math.max(8, Math.min(vh - h - bottomPad, y)),
      };
    },
  ...
  (function wireMiniInteractions() {
    ...
    const onPointerMove = (e) => {
      ...
      if (pinch.active) return; // a 2-finger gesture takes over — stop dragging
      const dx = e.clientX - startX, dy = e.clientY - startY;
      if (Math.abs(dx) + Math.abs(dy) > 4) moved = true;
      const w = host.offsetWidth, h = host.offsetHeight;
      let nx = origX + dx, ny = origY + dy;
      const pad = 8;
      const bottomNavH = mini._bottomNavH();
      const isMobile = window.innerWidth < 860 || (matchMedia && matchMedia('(pointer: coarse)').matches);
      const bottomPad = isMobile ? (bottomNavH + pad) : pad;
      nx = Math.max(pad, Math.min(window.innerWidth - w - pad, nx));
      ny = Math.max(pad, Math.min(window.innerHeight - h - bottomPad, ny));
      mini.miniPos = { x: nx, y: ny };
      // Direct write — no rAF needed because we're in a pointer event
      host.style.transform = `translate3d(${nx}px, ${ny}px, 0)`;
      e.preventDefault();
    };
    ...
    const onPointerUp = (e) => {
      ...
      if (moved) return; // it was a real drag, not a tap

      // YouTube-style tap detection:
      //   single tap → show controls (auto-hide after 2.4s)
      //   double tap → expand back to stage
      const now = Date.now();
  ...
  async function renderWatch(...args) {
    ...
    $('#view').innerHTML = `
      <section class="watch-page">
        <div class="watch-page__bar">
          ...
          <button class="icon-btn" id="watchMinimize" title="Minimize"><i class="ri-corner-down-right-line"></i></button>
    ...
    $('.watch-page__info').innerHTML = `
      <div>
        ...
        <a class="btn-ghost" href="/title/${type}/${id}" data-link><i class="ri-information-line"></i> About this title</a>
      </div>
      ${type === 'tv' ? '' : `
      <aside class="watch-page__aside">
        <h4>What's next</h4>
  ...
  const onSearch = debounce(async () => {
    ...
    // NSFW + anime fully unlocked in search.
    const data = await tmdb('/search/multi', { query: q, include_adult: 'true' });
  ...
  $('#pipToggle')?.addEventListener('click', () => {
    ...
    // If we're currently on the watch page, sync the Minimize button
    // visibility immediately so the user sees the effect without a reload.
    const minBtn = $('#watchMinimize');
    if (minBtn) minBtn.hidden = !next;
  });
  // Apply the saved preference to the <html> element on first load so
  // CSS can react (e.g. hiding pip-only chrome). Also surface the UI state.
